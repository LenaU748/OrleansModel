...
class Agent(Circle):
    ...
    def __init__(self, idh, idv, point=ORIGIN, **kwargs):
...
class DistrictOne(Scene):
    #python -m manim OrleansModel_1.0.py DistrictOne -pl
    def construct(self):
        random.seed(a=None, version=2)
        coords = [(1.8, 4),(3.6, 2.6),(2, 0.6),(2.2, -0.4),(2.4, -3.8), (1.8, -3.8),
                  (1.8, -2.2),(0.6, -1.6),(-0.6, -3.2),(-4.2, -0.2),(-1.6, 2.4),
                  (-0.8, 1.6),(0, 2.2),(0.2, 2)]
        ...
        disOnePoly = Polygon(
            ...
            np.array([-0.8, 1.6, 0]),
            np.array([0, 2.2, 0]),
            np.array([0.2, 2, 0]), color=WHITE)

        popCount = 30
        numAgents = []
   
        while len(numAgents) < popCount:
            x = random.uniform(-8, 8)
            y = random.uniform(-5, 5)
            dot = shapePoint(x, y)
            if dot.within(checkPoly):
                #agent = Agent(np.array([dot.x, dot.y, 0]))
                agent = Agent(idh=random.choice([-1, 1]), idv=random.choice([-1, 1]), point=[dot.x, dot.y, 0])
                numAgents.append(agent)
                self.add(agent)

        self.add(disOnePoly)
        text = TextMobject(str(len(numAgents)))
        self.add(text) 

        for n in range(20):
            for i in range(len(numAgents)):
                while True:
                    h = numAgents[i].idh*random.uniform(0, 0.05) + numAgents[i].point[0]
                    v = numAgents[i].idv*random.uniform(0, 0.05) + numAgents[i].point[1]
                    new_loc = shapePoint(h, v)

                    if new_loc.within(checkPoly):
                        numAgents[i].point[0] = h
                        numAgents[i].point[1] = v
                        numAgents[i].move_to(np.array([h, v, 0]))
                        break
                    else:
                       numAgents[i].idh *= -1
                       numAgents[i].idv *= -1
...
#THIS IS THE ORIGINAL SIR CODE FROM 3BLUE1BROWN
